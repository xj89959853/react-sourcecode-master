...
function placeChild(newFiber:Fiber,lastPlacedIndex:number,newIndex:number):number{
    ...
    // 判断旧节点存在，如果存在，则判断是否移动，不存在则插入
    if(current !== null){
        // A->B->C => [C,A,B] => C->A->B
        // 获取旧节点索引
        const oldIndex = current.index;
        // 判断旧节点索引是否小于lastPlacedIndex，如果小于，则移动，否则不移动
...
/**
 * 将剩余的子节点添加到临时映射，以便快速查找
 * @param currentFirstChild 
 * @returns Map<string,Fiber>
 */
function mapRemainingChildren(currentFirstChild:Fiber|null):Map<string,Fiber>{
    // 创建一个map
...
/**
 * 从映射中获取节点
 * @param existingChildren 剩余子节点映射
 * @param returnFiber 父节点
 * @param newIdx 新节点的索引
 * @param newChild 新节点
 * @returns Fiber
 */
function updateFromMap(existingChildren:Map<string,Fiber>,returnFiber:Fiber,newIdx:number,newChild:any):Fiber{
    // 判断文本类型
    if(typeof newChild === 'string' || typeof newChild === 'number'){
       // 从map中获取节点 A->B->C => [sC,sA]
...
function reconcileChildrenArray(returnFiber:Fiber,children:any):Fiber|null{
    ...
    // 第三阶段：Map查找，前两个阶段无法处理的情况，都在这个阶段处理，位置不同或完全不存在，A->B->C=>[B,C,A,D,E]
    // 将旧树上剩余的fiber转成map
