...
export function updateOnFiber(fiber:Fiber){
    ...
    if(hostRootFiber.child?.stateNode){
        removeChild(hostRootFiber.stateNode.containerInfo,hostRootFiber.child?.stateNode);
    }
    workInProgress=createWorkInProgress(hostRootFiber,hostRootFiber.pendingProps);
    workLoop();
    commitMutationEffects(hostRootFiber.alternate!);
}
