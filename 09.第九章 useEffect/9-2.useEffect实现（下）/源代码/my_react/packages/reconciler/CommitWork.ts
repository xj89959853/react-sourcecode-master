...
import { Flags,MutationMask, Placement, Update,ChildDeletion,PassiveEffect } from "./FiberFlags";
import { detachDeletedInstance } from "packages/react-dom-binding/ReactDOMComponentTree";
import { HasEffect } from "./FiberHook";
// 最近的宿主父节点
...
function detachAlternateSibling(finishedWork:Fiber){
    ...
    if(previousFiber){
        ...
        if(detachedChild){
            ...
            do{
                const sibling:Fiber|null = detachedChild.sibling;
...
function commitPassiveUnmountEffectsInsideOfDeletedTree_begin(childToDelete:Fiber){
    while(nextEffect){
        commitPassiveUnmountInsideDeletedTreeOnFiber(nextEffect);
        if(nextEffect.child){
